...
Student Registration Form submission with valid data and assert entry - This test is to ensure that I can successfully submit data to the submission form. Once again I am storing data in the testData.json. I don't believe this is a fragility as this data would not be stored on the product.
...
Student Registration form required fields validation and assert CSS border colour - This test is to ensure that the data validation is working as I would imagine it to. This is the test that is currently failing because the form allows some fields to be empty. Some of these fields, I'd argue need to be filled like Subject. For more information see, DEFECTS.md
...
For futher detail on the bug see DEFECTS.md. Also in this folder, there is a screenshot of the failure. Which shows the inputs that are showing the incorrect data validation colour. 
...
Test Data - Some of the test data stored in this suite is static. This is a fragility because if the data stored on the web page changes. The tests will fail. If I was able to, I would retrieve the data from the database. 
